  ...
  def parse_multiple(itemlist, readability_key) do
      ...
      items = Parallel.map itemlist, fn item ->
          case parse item[:link], readability_key do
              {:ok, title, content} ->
                  [id: item[:id], title: xml_quote(title), original_post: xml_quote(item[:original_post]), summary: xml_quote("<hr><h2>" <> title <> "</h2>" <> content), link: xml_quote(item[:link]), clients: item[:clients]]
              _ -> []
          end
      end
      case items do
